...
function getAnimalEmojiForUser(email) {
    ...
    const emoji = animalEmojis[index];
...
async function migrateAlbumsWithTracks() {
    ...
    const confirmMigrate = await showCustomConfirm('This will fetch and add track listings to all existing albums that have YouTube Music links. Continue?', 'Migrate Tracks');
    ...
    try {
        ...
        await showCustomAlert(`Migration complete!\nUpdated: ${updated}\nSkipped: ${skipped}\nFailed: ${failed}`, 'Migration Complete');
        ...
        await showCustomAlert(`Migration failed: ${error.message}`, 'Migration Failed');
    }
}
...
async function loadModerators() {
    try {
        ...
        if (doc.exists && doc.data().emails) {
            ...
            console.log('Moderators loaded:', MODERATORS);
        }
    } catch (error) {
...
async function handleCredentialResponse(response) {
    // Google has already verified the JWT signature
    // We can safely decode it - DO NOT use parseJwt for untrusted tokens
    const payload = parseJwt(response.credential);
    
    // Verify the token is from Google and hasn't expired
    ...
    if (!ALLOWED_USERS.includes(payload.email)) {
        await showCustomAlert('Access denied. Your email is not authorized to use this application.', 'Access Denied');
    ...
    try {
        ...
        // Continue anyway - the app will work but Firestore writes might fail
    }
    
    currentUser = {
        name: sanitizeText(payload.name),
        email: sanitizeText(payload.email),
        picture: payload.picture // URLs are safe for img src
    };
    
    // Store minimal session info (user can't forge this without re-auth)
...
function showMainContent() {
    ...
    document.getElementById('userImage').src = currentUser.picture;
...
function showCustomAlert(message, title = 'Musicframe') {
    return new Promise((resolve) => {
        ...
        // Single OK button
        buttonsContainer.innerHTML = '<button class="modal-btn modal-btn-primary" id="customAlertOk">OK</button>';
...
function setupEventListeners() {
    ...
    document.getElementById('leaveDiscussionBtn')?.addEventListener('click', leaveDiscussion);
    document.getElementById('addAlbumBtn')?.addEventListener('click', showAddAlbumModal);
    document.getElementById('migrateTracksBtn')?.addEventListener('click', migrateAlbumsWithTracks);
    document.getElementById('startVotingBtn')?.addEventListener('click', showVotingModal);
    document.getElementById('submitVotesBtn')?.addEventListener('click', submitVotes);
    document.getElementById('endVotingBtn')?.addEventListener('click', showEndVotingConfirmModal);
    document.getElementById('resetVotingBtn')?.addEventListener('click', showResetVotingConfirmModal);
    
    // Album Modal event listeners
    document.getElementById('closeAddAlbumModal')?.addEventListener('click', closeAddAlbumModal);
    document.getElementById('cancelAddAlbum')?.addEventListener('click', closeAddAlbumModal);
    document.getElementById('confirmAddAlbum')?.addEventListener('click', processAddAlbum);
    ...
    // Submit on Enter key
    document.getElementById('albumLinkInput')?.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            processAddAlbum();
        }
    });
    ...
    document.getElementById('cancelLock')?.addEventListener('click', closeLockConfirmModal);
    document.getElementById('confirmLock')?.addEventListener('click', processLockAlbum);
    ...
    document.getElementById('closeUnlockConfirmModal')?.addEventListener('click', closeUnlockConfirmModal);
    document.getElementById('cancelUnlock')?.addEventListener('click', closeUnlockConfirmModal);
    document.getElementById('confirmUnlock')?.addEventListener('click', processUnlockAlbum);
    
    // Close modal on overlay click
    document.getElementById('unlockConfirmModal')?.addEventListener('click', (e) => {
    ...
    document.getElementById('closeLockedInfoModal')?.addEventListener('click', closeLockedInfoModal);
    document.getElementById('closeLockedInfoBtn')?.addEventListener('click', closeLockedInfoModal);
    ...
    document.getElementById('closeRateTrackModal')?.addEventListener('click', closeRateTrackModal);
    document.getElementById('cancelRateTrack')?.addEventListener('click', closeRateTrackModal);
    document.getElementById('rateFavorite')?.addEventListener('click', () => applyTrackRating('favorite'));
    document.getElementById('rateLeast')?.addEventListener('click', () => applyTrackRating('least'));
    document.getElementById('rateLiked')?.addEventListener('click', () => applyTrackRating('liked'));
    document.getElementById('rateDisliked')?.addEventListener('click', () => applyTrackRating('disliked'));
    ...
    document.getElementById('closeEditLinksModal')?.addEventListener('click', closeEditLinksModal);
    document.getElementById('cancelEditLinks')?.addEventListener('click', closeEditLinksModal);
    document.getElementById('saveEditLinks')?.addEventListener('click', saveEditedLinks);
    document.getElementById('addNewLinkBtn')?.addEventListener('click', addNewLinkRow);
    ...
    document.getElementById('closeCustomAlert')?.addEventListener('click', closeCustomAlert);
    document.getElementById('confirmCustomAlert')?.addEventListener('click', closeCustomAlert);

    // Voting warning modal event listeners
    document.getElementById('closeVotingWarning')?.addEventListener('click', closeVotingWarningModal);
    document.getElementById('cancelVotingWarning')?.addEventListener('click', closeVotingWarningModal);
    document.getElementById('confirmVotingStart')?.addEventListener('click', showVotingConfirmModal);

    // Second voting confirmation modal event listeners
    document.getElementById('closeVotingConfirm')?.addEventListener('click', closeVotingConfirmModal);
    document.getElementById('cancelVotingConfirm')?.addEventListener('click', closeVotingConfirmModal);
    document.getElementById('confirmVotingGo')?.addEventListener('click', confirmVotingStart);
    
    // Reset voting confirmation modal event listeners
    document.getElementById('closeResetVotingConfirm')?.addEventListener('click', closeResetVotingConfirmModal);
    document.getElementById('cancelResetVoting')?.addEventListener('click', closeResetVotingConfirmModal);
    document.getElementById('confirmResetVoting')?.addEventListener('click', confirmResetVoting);
    
    // End voting confirmation modal event listeners
    document.getElementById('closeEndVotingConfirm')?.addEventListener('click', closeEndVotingConfirmModal);
    document.getElementById('cancelEndVoting')?.addEventListener('click', closeEndVotingConfirmModal);
    document.getElementById('confirmEndVoting')?.addEventListener('click', confirmEndVoting);

    // Leave discussion modal event listeners
    document.getElementById('closeLeaveDiscussion')?.addEventListener('click', closeLeaveDiscussionModal);
    document.getElementById('cancelLeaveDiscussion')?.addEventListener('click', closeLeaveDiscussionModal);
    document.getElementById('confirmLeaveDiscussion')?.addEventListener('click', confirmLeaveDiscussion);

    // Voting Modal event listeners
    document.getElementById('closeVotingModal')?.addEventListener('click', closeVotingModal);
    document.getElementById('cancelVoting')?.addEventListener('click', closeVotingModal);
...
async function showEditLinksModal(albumId) {
    ...
    youtubeInfo.textContent = 'This is your main album link. Once locked, it cannot be changed. Only a moderator can unlock it.';
    ...
    if (isLocked) {
        ...
        lockNote.textContent = '🔒 Album is locked - YouTube Music link cannot be changed';
...
function showLockedInfoModal() {
    ...
    if (moderatorNames.length === 1) {
        ...
        const namesList = moderatorNames.slice(0, -1).join(', ') + ' or ' + moderatorNames[moderatorNames.length - 1];
...
async function joinDiscussion() {
    ...
    try {
        ...
        const participantRef = db.collection('participants').doc(`${monthKey}_${currentUser.email}`);
        ...
        await showCustomAlert('Failed to join discussion. Please try again.', 'Error');
    }
}
...
async function leaveDiscussion() {
    ...
    try {
        ...
        if (!albumsSnapshot.empty) {
            await showCustomAlert('You must delete your album before leaving the discussion.');
            return;
        }
...
async function confirmLeaveDiscussion() {
    ...
    try {
        ...
        await showCustomAlert('You have left the discussion successfully.', 'Success');
    } catch (error) {
        console.error('Error leaving discussion:', error);
        await showCustomAlert('Failed to leave discussion. Please try again.', 'Error');
    }
}
...
async function removeParticipant(participantEmail, participantName) {
    ...
    const confirmed = await showCustomConfirm(`Are you sure you want to remove ${firstName} from this discussion?`, 'Remove Participant');
    if (!confirmed) {
        return;
    }
    ...
    try {
        const participantRef = db.collection('participants').doc(`${monthKey}_${participantEmail}`);
        ...
        await loadMonthData(currentDate);
        await showCustomAlert(`${firstName} has been removed from the discussion.`, 'Success');
    } catch (error) {
        console.error('Error removing participant:', error);
        await showCustomAlert('Failed to remove participant. Please try again.', 'Error');
    }
}
...
async function loadParticipants(monthKey) {
    ...
    try {
        ...
        participants.sort((a, b) => {
            ...
            const timeA = a.joinedAt.toMillis ? a.joinedAt.toMillis() : new Date(a.joinedAt).getTime();
            const timeB = b.joinedAt.toMillis ? b.joinedAt.toMillis() : new Date(b.joinedAt).getTime();
            return timeA - timeB;
        });
...
async function loadAlbums(monthKey) {
    ...
    try {
        // Check voting state for this month (with error handling)
        // Check if any album has votingActive flag
        ...
        // Sort albums by the order participants joined
        // Get participant order from global participants list
        if (window.currentParticipants && window.currentParticipants.length > 0) {
            const participantOrder = window.currentParticipants.map(p => p.email);
            albums.sort((a, b) => {
                const indexA = participantOrder.indexOf(a.addedBy);
                const indexB = participantOrder.indexOf(b.addedBy);
                // If both found, sort by participant order
...
function updateAddAlbumButton(albums) {
    ...
    const isParticipant = window.currentParticipants && 
...
async function deleteAlbum(albumId) {
    ...
    if (albumData.locked && !userIsModerator) {
        await showCustomAlert('This album is locked. Only a moderator can delete it.');
        return;
    }
    ...
    const confirmMessage = userIsModerator && !isOwnAlbum 
        ? `Are you sure you want to delete this album by ${albumData.addedByName || 'another user'}?`
...
async function showAddAlbumModal() {
    ...
    if (snapshot.empty) {
        await showCustomAlert('You must join this month\'s discussion before adding an album!');
        return;
    }
...
async function processAddAlbum() {
    ...
    if (!playlistIdMatch) {
        alert('Invalid YouTube Music link. Please make sure it contains a playlist ID.');
        return;
    }
    ...
    try {
        ...
        if (!data.items || data.items.length === 0) {
            await showCustomAlert('Could not find album information. Please try again.');
            return;
        }
        ...
        let thumbnail = playlist.snippet.thumbnails.maxres?.url ||
        ...
        await showCustomAlert(`Failed to fetch album information: ${error.message}\n\nPlease enter details manually.`, 'Error');
...
async function addAlbum(title, artist, coverUrl, youtubeUrl = '', playlistId = '', tracks = []) {
    if (!db || !currentUser) {
    ...
    try {
        ...
        // Add the album
        console.log('Adding album:', { title, artist, coverUrl, youtubeUrl, playlistId, tracks, monthKey });
        ...
        await showCustomAlert(`Failed to add album: ${error.message}\n\nPlease check the browser console for details.`, 'Error');
    }
}
...
function updateJoinButton(participants) {
    ...
    const isParticipating = currentUser && participants.some(p => p.email === currentUser.email);
...
function updateParticipants(participants) {
    ...
    // Define unique colors for each participant
    ...
    participants.forEach((participant, index) => {
        ...
        if (currentUser && isModerator(currentUser.email) && participant.email !== currentUser.email) {
            const removeBtn = document.createElement('button');
...
function updateAlbums(albums) {
    ...
    if (albums.length === 0) {
        ...
        p.style.color = '#666';
        p.textContent = 'No albums added yet. Join the discussion and be the first!';
    ...
    // Random gradients for album covers
    const gradients = [
    ...
    sortedAlbums.forEach((album, index) => {
        ...
        const participant = document.querySelector(`.participant[data-email="${album.addedBy}"]`);
        ...
        if (isUserAlbum || userIsModerator) {
            ...
            if (!album.locked || userIsModerator) {
                ...
                deleteBtn.title = userIsModerator && !isUserAlbum ? 'Delete album (Moderator)' : 'Delete your album';
        ...
        console.log('Album:', album.title, 'votingActive:', votingActive, 'isUserAlbum:', isUserAlbum, 'userHasVoted:', userHasVoted, 'window.currentVotingActive:', window.currentVotingActive);
        ...
        if (votingActive && album.votes && Object.keys(album.votes).length > 0) {
            ...
            Object.entries(album.votes).forEach(([voterEmail, points]) => {
                const participant = window.currentParticipants?.find(p => p.email === voterEmail);
                if (participant) {
                    // Get participant's color
                    const participantElement = document.querySelector(`.participant[data-email="${voterEmail}"]`);
        ...
        else if (!votingActive && album.votes && Object.keys(album.votes).length > 0) {
            const totalVotes = Object.values(album.votes).reduce((sum, points) => sum + points, 0);
        ...
        if (album.externalLinks && album.externalLinks.length > 0 && !votingActive) {
            ...
            album.tracks.forEach(track => {
                ...
                Object.entries(trackRatings).forEach(([userEmail, userRating]) => {
                    ...
                    if (ratingType) {
                        ...
                        const participant = window.currentParticipants?.find(p => p.email === userEmail);
                        if (participant) {
...
function highlightParticipant(email) {
    const participant = document.querySelector(`.participant[data-email="${email}"]`);
    if (participant) {
...
async function showRateTrackModal(albumId, track, allTrackRatings) {
    ...
    document.querySelectorAll('.rating-btn .rating-avatars').forEach(el => el.remove());
        ...
        Object.entries(ratingTypes).forEach(([ratingType, buttonId]) => {
            ...
            if (usersWithRating.length > 0) {
                ...
                usersWithRating.forEach(email => {
                    const participant = window.currentParticipants?.find(p => p.email === email);
                    if (participant) {
                        const avatar = document.createElement('img');
                        avatar.src = participant.picture;
                        avatar.alt = participant.name;
...
async function applyTrackRating(ratingType) {
    ...
    if (ratingType === 'favorite') {
        ...
        if (liked.includes(trackKey)) {
            ...
            if (newUserRating.disliked) {
                newUserRating.disliked = newUserRating.disliked.filter(k => k !== trackKey);
            }
        }
    ...
    try {
        ...
        if (error.code === 'permission-denied') {
            showCustomAlert('Permission denied. You need to be signed in to rate tracks.');
        } else {
...
function toggleHighlightAlbum(email) {
    ...
    if (isHighlighted) {
        ...
        document.querySelectorAll('.participant.highlighted').forEach(p => p.classList.remove('highlighted'));
...
async function showVotingModal() {
    ...
    if (!isParticipant) {
        showCustomAlert('Only participants can vote. Please join the discussion first.');
        return;
    }
    ...
    if (albumCount < 2) {
        showCustomAlert('Voting requires at least 2 other albums (you need at least 3 total participants).');
        return;
    }
    ...
    if (unlockedAlbums.length > 0) {
        ...
        showCustomAlert(`All albums must be locked before voting can start.\n\nParticipants with unlocked albums: ${participantList}`);
        return;
    }
...
async function confirmResetVoting() {
    ...
    try {
        ...
        showCustomAlert('All votes have been reset. You can now start voting again.');
...
async function selectVote(albumId, points) {
    ...
    if (submitBtn && currentMonthVotes.threePoints && currentMonthVotes.onePoint) {
...
function updateVoteUI() {
    document.querySelectorAll('.album-vote-circle').forEach(circle => circle.classList.remove('selected'));
    
    if (currentMonthVotes.threePoints) {
        const circle3 = document.querySelector(`.album-vote-circle.vote-3[data-album-id="${currentMonthVotes.threePoints}"]`);
        circle3?.classList.add('selected');
    }
    
    if (currentMonthVotes.onePoint) {
        const circle1 = document.querySelector(`.album-vote-circle.vote-1[data-album-id="${currentMonthVotes.onePoint}"]`);
...
async function submitVotes() {
    if (!currentMonthVotes.threePoints || !currentMonthVotes.onePoint) {
        showCustomAlert('Please select both votes (3 points and 1 point) before submitting.');
        return;
    }
    ...
    try {
        ...
        // Check if user has already voted
        const vote3Album = db.collection('albums').doc(currentMonthVotes.threePoints);
...
async function loadAlbums(monthKey) {
    ...
    try {
        ...
        window.currentVotingActive = votingActive;
        console.log('loadAlbums: Set window.currentVotingActive to', votingActive, 'from', albums.length, 'albums');
        ...
        if (votingStatusMessage && votingActive && userHasVoted && currentUser) {
            ...
            if (waitingOn.length > 0) {
                votingStatusMessage.textContent = `⏳ Waiting on ${waitingOn.join(', ')} to cast their vote${waitingOn.length > 1 ? 's' : ''}`;
        ...
        if (submitBtn && startBtn && endBtn && resetBtn) {
            if (votingActive && !userHasVoted) {
                ...
                const hasAnyVotes = albums.some(album => album.votes && Object.keys(album.votes).length > 0);
                ...
                const canStartVoting = !hasAnyVotes && participantCount >= 2 && albums.length >= 2;
                startBtn.style.display = canStartVoting ? 'inline-block' : 'none';
                
                // Show reset button if there are votes to reset (moderator only)
                resetBtn.style.display = (userIsModerator && hasAnyVotes) ? 'inline-block' : 'none';
            }
        }
